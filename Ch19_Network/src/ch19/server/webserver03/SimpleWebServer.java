...
public class SimpleWebServer {
	public static void main(String[] args) {
		final int PORT = 80; // http는 디폴트로 80 포트 사용
...
//			System.out.println("line" + line); // 필요에 따라 함께 추가해서 디버깅
			int start = line.indexOf(" ") + 2;
			int end = line.lastIndexOf("HTTP") - 1; //HTTP는 작성
//			System.out.println("start=" + start + "end=" + end); // 필요에 따라 함께 추가해서 디버깅
			String fileName = line.substring(start, end);
//			System.out.println("fileName=" + fileName);  // 필요에 따라 함께 추가해서 디버깅
			if(fileName.equals(""))
				fileName = "index.html";
